...
// Two-company (QP order-process audit, 2026-08-25): app_settings is a
// single global key/value store with no company_name_id -- every document
// used to print whatever this one global "company_legal_name" was set to
// (defaulting to "Sakshi Creation"), regardless of which company the
// document actually belonged to. Every document this file builds already
// carries its own linked company (quotation.companyName / invoice.companyName
// / challan.companyName, all populated from the same company_names join),
// so the letterhead now prefers that real, per-document company name and
// only falls back to the global setting for a document with no company
// link at all (company_name_id is nullable on all three tables).
function letterheadName(doc, globalName) {
  return doc?.companyName?.companyName || globalName;
}
...
async function buildQuotationPdf(quotation) {
  const companyName = letterheadName(quotation, await getCompanyLegalName());
...
async function buildInvoicePdf(invoice, items) {
  const companyName = letterheadName(invoice, await getCompanyLegalName());
  const doc = new PDFDocument({ margin: 40, size: "A4" });
...
async function buildDeliveryChallanPdf(challan) {
  const companyName = letterheadName(challan, await getCompanyLegalName());
  const doc = new PDFDocument({ margin: 40, size: "A4" });
